...
func testHelperGetDeviceWithBestEffort(testName string, t *testing.T) *skywallet.Device {
	emDevice := skywallet.NewDevice(skywallet.DeviceTypeEmulator)
	usbDevice := skywallet.NewDevice(skywallet.DeviceTypeUSB)
...
func TestDevice(t *testing.T) {
	...
	if device.Driver.DeviceType() == skywallet.DeviceTypeEmulator &&
...
func TestGetDeviceEntropyShouldWorkOk(t *testing.T) {
	...
	generators := [...]func(entropyBytes uint32) ([][64]byte, error){
		skywallet.MessageDeviceGetRawEntropy,
		skywallet.MessageDeviceGetMixedEntropy,
	}
...
func TestGetAddressEmulator(t *testing.T) {
	device := skywallet.NewDevice(skywallet.DeviceTypeEmulator)
	require.NotNil(t, device)
	if err := device.Connect(); err != nil {
		log.Error(err)
	}
